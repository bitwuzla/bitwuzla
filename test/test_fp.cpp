...
class TestFpInternal : public TestBzla
{
 protected:
  ...
  void test_to_fp_from_rational(
      BzlaRoundingMode rm, std::vector<std::vector<const char *>> &expected)
  {
    BzlaMemMgr *mm = d_bzla->mm;
    BzlaFloatingPoint *fp;
    BzlaBitVector *sign, *exp, *sig;
    ...
    assert(d_constants_rat.size() == expected.size());
    for (size_t i = 0, n = d_constants_rat.size(); i < n; ++i)
    {
      fp = bzla_fp_convert_from_rational(d_bzla,
                                         d_f16,
                                         rm,
                                         d_constants_rat[i].first,
                                         d_constants_rat[i].second);
      bzla_fp_as_bvs(d_bzla, fp, &sign, &exp, &sig);
      sign_str = bzla_bv_to_char(mm, sign);
      exp_str  = bzla_bv_to_char(mm, exp);
      sig_str  = bzla_bv_to_char(mm, sig);
...
TEST_F(TestFpInternal, fp_from_real_rat_str_rna)
{
  ...
  test_to_fp_from_rational(BzlaRoundingMode::BZLA_RM_RNA, expected);
}
...
TEST_F(TestFpInternal, fp_from_real_rat_str_rne)
{
  ...
  test_to_fp_from_rational(BzlaRoundingMode::BZLA_RM_RNE, expected);
}
...
TEST_F(TestFpInternal, fp_from_real_rat_str_rtn)
{
  ...
  test_to_fp_from_rational(BzlaRoundingMode::BZLA_RM_RTN, expected);
}
...
TEST_F(TestFpInternal, fp_from_real_rat_str_rtp)
{
  ...
  test_to_fp_from_rational(BzlaRoundingMode::BZLA_RM_RTP, expected);
}
...
TEST_F(TestFpInternal, fp_from_real_rat_str_rtz)
{
  ...
  test_to_fp_from_rational(BzlaRoundingMode::BZLA_RM_RTZ, expected);
}
