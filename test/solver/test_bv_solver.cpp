...
TEST_F(TestBvSolver, solve_eq1)
{
  ...
  Node one = nm.mk_value(BitVector(8, 1ul));
...
TEST_F(TestBvSolver, value1)
{
  ...
  ASSERT_EQ(ctx.solve(), Result::SAT);
  ASSERT_EQ(ctx.get_value(x), nm.mk_value(BitVector(8, 0ul)));
}
...
TEST_F(TestBvSolver, value2)
{
  ...
  ctx.assert_formula(
      nm.mk_node(Kind::EQUAL, {x, nm.mk_value(BitVector(8, 2ul))}));
  ctx.assert_formula(
      nm.mk_node(Kind::EQUAL, {y, nm.mk_value(BitVector(8, 5ul))}));
  ASSERT_EQ(ctx.solve(), Result::SAT);
  ASSERT_EQ(ctx.get_value(nm.mk_node(Kind::BV_MUL, {x, y})),
            nm.mk_value(BitVector(8, 10ul)));
}
...
TEST_F(TestBvSolver, solve1)
{
  ...
  Node concat =
      nm.mk_node(Kind::BV_CONCAT, {nm.mk_value(BitVector(6, 0ul)), x});

  ctx.assert_formula(nm.mk_node(
      Kind::EQUAL,
      {concat,
       nm.mk_node(Kind::BV_SHL, {nm.mk_value(BitVector(8, 1ul)), concat})}));
  ASSERT_EQ(ctx.solve(), Result::UNSAT);
}
