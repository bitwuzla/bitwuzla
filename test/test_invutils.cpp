...
using CreateBinExp = BoolectorNode *(Bzla *, BoolectorNode *, BoolectorNode *);
...
class TestInvUtils : public TestMm
{
 protected:
  ...
  void test_is_inv_const(BzlaPropIsInv is_inv,
                        ...
                        uint32_t pos_x)
  {
    ...
    if (pos_x == 0)
      exp = create_exp(bzla, nx, ns);
    else
    {
      assert(pos_x == 1);
      exp = create_exp(bzla, ns, nx);
    }
