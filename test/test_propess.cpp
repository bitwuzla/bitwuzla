...
class TestPropEssential : public TestPropCommon
{
 protected:
  void test_binary(BzlaBinFun expr_fun,
                   BzlaPropIsEssFun ess_fun,
                   uint32_t pos_x,
                   bool fixed_bits)
  {
    uint32_t result, expected_result, pos_s;
    Bzla *bzla;
      ...
      for (const std::string &sval : values_s)
      {
        ...
        for (const std::string &tval : values_t)
        {
          ...
          while (bzla_bvdomain_gen_has_next(&gen))
          {
            ...
            bzla->slv = slv_prop;
            /* check if s is essential wrt to constant bits in x (if any) */
            result = ess_fun(bzla, &pi, pos_s);

            expected_result = result ? BZLA_RESULT_SAT : BZLA_RESULT_UNSAT;
            ...
            if (res != expected_result)
            {
              ...
              std::cout << "ess_s: " << result << std::endl;
              std::cout << "bv_x: ";
              if (bv_x)
              {
                bzla_bv_print(bv_x);
...
TEST_F(TestPropCons, ess_add)
{
  test_binary(bzla_exp_bv_add, bzla_is_ess_add, 0, false);
  test_binary(bzla_exp_bv_add, bzla_is_ess_add, 1, false);
}
