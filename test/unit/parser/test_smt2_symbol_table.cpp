...
TEST_F(TestSmt2SymbolTable, insert)
{
  ...
  ASSERT_EQ(x->d_scope_level, 0);
  ASSERT_EQ(y->d_symbol, "y");
  ASSERT_EQ(y->d_scope_level, 0);
  table.insert(Token::SYMBOL, "x", 1);
  SymbolTable::Node* x1 = table.find("x");
  ASSERT_NE(x, x1);
  ASSERT_EQ(x1->d_symbol, "x");
  ASSERT_EQ(x1->d_scope_level, 1);
  table.remove(x1);
  x1 = table.find("x");
  ASSERT_EQ(x, x1);
  ASSERT_EQ(x1->d_symbol, "x");
  ASSERT_EQ(x1->d_scope_level, 0);
  table.remove(x1);
...
TEST_F(TestSmt2SymbolTable, insert_quoted1)
{
  ...
  ASSERT_EQ(x_new->d_scope_level, 2);
  ...
  ASSERT_EQ(x_quoted->d_symbol, "x");
  ASSERT_EQ(x_quoted->d_scope_level, 0);
...
TEST_F(TestSmt2SymbolTable, insert_quoted2)
{
  ...
  ASSERT_EQ(y_quoted->d_symbol, "|y|");
  ASSERT_EQ(y_quoted->d_scope_level, 0);
  table.remove(x_new);
  ASSERT_EQ(table.find(" x"), nullptr);
}
}  // namespace bzla::test
