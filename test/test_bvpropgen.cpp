...
{
 protected:
  void test_next(const char *str_d, std::vector<std::string> expected)
  {
...
TEST_F(TestBvPropGen, has_next)
{
  ...
  for (bw = 1; bw <= 8; bw++)
  {
    ...
    for (i = 0; i < num_consts; i++)
    {
      ...
      while (bzla_bvprop_gen_has_next(&gen))
      {
        bzla_bv_free(d_mm, bzla_bvprop_gen_next(&gen));
      }
      bzla_bvprop_free(d_mm, d);
      bzla_bvprop_gen_delete(&gen);
    }
...
TEST_F(TestBvPropGen, next_00x) { test_next("00x", {"000", "001"}); }
TEST_F(TestBvPropGen, next_01x) { test_next("01x", {"010", "011"}); }
TEST_F(TestBvPropGen, next_10x) { test_next("10x", {"100", "101"}); }
TEST_F(TestBvPropGen, next_11x) { test_next("11x", {"110", "111"}); }

TEST_F(TestBvPropGen, next_0x0) { test_next("0x0", {"000", "010"}); }
TEST_F(TestBvPropGen, next_0x1) { test_next("0x1", {"001", "011"}); }
TEST_F(TestBvPropGen, next_1x0) { test_next("1x0", {"100", "110"}); }
TEST_F(TestBvPropGen, next_1x1) { test_next("1x1", {"101", "111"}); }

TEST_F(TestBvPropGen, next_x00) { test_next("x00", {"000", "100"}); }
TEST_F(TestBvPropGen, next_x01) { test_next("x01", {"001", "101"}); }
TEST_F(TestBvPropGen, next_x10) { test_next("x10", {"010", "110"}); }
TEST_F(TestBvPropGen, next_x11) { test_next("x11", {"011", "111"}); }

TEST_F(TestBvPropGen, next_0xx)
{
  test_next("0xx", {"000", "001", "010", "011"});
}

TEST_F(TestBvPropGen, next_1xx)
{
  test_next("1xx", {"100", "101", "110", "111"});
}

TEST_F(TestBvPropGen, next_xx0)
{
  test_next("xx0", {"000", "010", "100", "110"});
}

TEST_F(TestBvPropGen, next_xx1)
{
  test_next("xx1", {"001", "011", "101", "111"});
}

TEST_F(TestBvPropGen, next_xxx)
{
  test_next("xxx", {"000", "001", "010", "011", "100", "101", "110", "111"});
}
