...
def test_term_outlives_explicit_termmanager_drop():
    """The Term's strong shared_ptr to the C++ TermManager must keep the
    underlying NodeManager alive even after every Python-level reference
    to the TermManager has been dropped."""
    tm = TermManager()
    rm = tm.mk_rm_value(RoundingMode.RNE)
    del tm
    gc.collect()
