...
class TestEssUtils : public TestBzla
{
 protected:
  ...
  void test_is_ess_cond (uint32_t pos_x, bool const_bits)
  {
    ...
    char *vs0, *vs1, *vt;
    bool res, status;
    uint32_t bw_s0, bw_s1, bw = TEST_ESSUTILS_BW;
      ...
      for (uint32_t i = 0; i < nval_s0; i++)
      {
        ...
        for (uint32_t j = 0; j < nval_s1; j++)
        {
          ...
          for (uint32_t k = 0; k < nval; k++)
          {
            ...
            {
              res = bzla_is_ess_cond (d_bzla, &pi);
            }
  ...
  void test_is_ess_slice (BzlaPropIsInvFun is_ess, bool const_bits)
  {
    std::vector<std::string> x_values;
    BzlaBvDomain *x;
    BzlaBitVector *t;
...
#if 0
  bool check_sat_is_ess_cond (BzlaBvDomain *x,
                              BzlaBitVector *t,
                              BzlaBitVector *s0,
                              BzlaBitVector *s1,
                              uint32_t pos_x)
  ...
  bool check_sat_is_ess_slice (BzlaBvDomain *x,
                               BzlaBitVector *t,
                               uint32_t upper,
                               uint32_t lower)
  {
