(set-logic QF_ABV)
(set-info :status unsat)
(declare-const a0 (Array (_ BitVec 1) (_ BitVec 1) ))
(declare-const a1 (Array (_ BitVec 1) (_ BitVec 1) ))
(declare-const v0 (_ BitVec 1))
(declare-const v1 (_ BitVec 1))
(declare-const v2 (_ BitVec 1))
(declare-const v3 (_ BitVec 1))
(assert (= #b1 (bvand (bvand (ite (= (ite (bvult v0 v1) #b1 #b0) (ite (bvult v2 v3) #b1 #b0)) #b1 #b0) (bvand (ite (bvult (select a0 v0) (select a0 v1)) #b1 #b0) (ite (bvult (select a1 v2) (select a1 v3)) #b1 #b0))) (ite (distinct a0 a1) #b1 #b0))))
(check-sat)
