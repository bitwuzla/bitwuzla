...
typedef struct BzlaSMT2Parser
{
  ...
  BzlaParseResult *res;
  BitwuzlaTermStack sat_assuming_assumptions;
  uint32_t scope_level;
  struct
  {
    uint32_t size, count;
...
static Bitwuzla *
get_bitwuzla(BzlaSMT2Parser *parser)
{
  if (!parser->bitwuzla)
  {
...
{
  // char *track  = 0;
  BitwuzlaOptions *options = parser->options;
...
static int64_t
str2uint64_smt2(BzlaSMT2Parser *parser,
                bool allow_zero,
                const char *str,
                uint64_t *resptr)
{
  ...
  for (p = str; (ch = *p); p++)
  {
    if (res > UINT64_MAX / 10 || ch < '0' || ch > '9')
    INVALID:
      return !perr_smt2(parser, "invalid 64-bit integer '%s'", str);
    assert('0' <= ch && ch <= '9');
    if (res) res *= 10;
    digit = ch - '0';
    if (UINT64_MAX - digit < res) goto INVALID;
    res += digit;
  }
  if (!allow_zero && !res)
    return !perr_smt2(
        parser, "expected positive non-zero 64-bit integer at '%s'", str);
...
static int64_t
parse_uint64_smt2(BzlaSMT2Parser *parser, bool allow_zero, uint64_t *resptr)
{
  int32_t tag = read_token_smt2(parser);
  if (tag == BZLA_INVALID_TAG_SMT2) return 0;
  if (tag == EOF)
  ...
  return str2uint64_smt2(parser, allow_zero, parser->token.start, resptr);
}
                         ...
                         bool check_fp)
{
  uint32_t i;
  (void) check_fp;
  std::string is_str, expected_str;
  bool is_err;
...
static int64_t
parse_bit_width_smt2(BzlaSMT2Parser *parser, uint64_t *width)
{
  int32_t tag;
  ...
  *width = 0;
  return str2uint64_smt2(parser, true, parser->token.start, width) ? 1 : 0;
}
                    ...
                    BitwuzlaKind kind)
{
  ...
  exp = bitwuzla_mk_term(kind, nargs, args.start);
                        ...
                        BitwuzlaKind kind)
{
  ...
  exp = bitwuzla_mk_term1(kind, item_cur[1].exp);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
  return 1;
}
                                   ...
                                   BitwuzlaKind kind)
{
  ...
  BZLA_INIT_STACK(parser->mem, args);
  for (uint32_t i = 1; i <= nargs; i++) BZLA_PUSH_STACK(args, item_cur[i].exp);
  BitwuzlaTerm exp = bitwuzla_mk_term(kind, nargs, args.start);
  BZLA_RELEASE_STACK(args);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
                      ...
                      BitwuzlaKind kind)
{
  ...
  if (!check_bv_args_smt2(parser, item_cur, nargs)) return 0;
  BitwuzlaTerm exp = bitwuzla_mk_term2(kind, item_cur[1].exp, item_cur[2].exp);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
  return 1;
}
                         ...
                         BitwuzlaKind kind)
{
  ...
  BitwuzlaTerm exp =
      bitwuzla_mk_term1_indexed1(kind, item_cur[1].exp, item_cur->num);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
  return 1;
}
                         ...
                         BitwuzlaKind kind)
{
  ...
  uint32_t width = bitwuzla_term_bv_get_size(item_cur[1].exp);
  BitwuzlaTerm exp =
      bitwuzla_mk_term1_indexed1(kind, item_cur[1].exp, item_cur->num % width);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
  return 1;
}
                        ...
                        BitwuzlaKind kind)
{
  ...
  BitwuzlaTerm exp = bitwuzla_mk_term1(kind, item_cur[1].exp);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
  return 1;
}
                         ...
                         BitwuzlaKind kind)
{
  ...
  BitwuzlaTerm exp = bitwuzla_mk_term3(
      kind, item_cur[1].exp, item_cur[2].exp, item_cur[3].exp);
  release_exp_and_overwrite(parser, item_open, item_cur, exp);
  return 1;
}
                          ...
                          uint32_t nargs)
{
  ...
  else
  {
    ...
    uint64_t idxs[]     = {item_cur->idx0, item_cur->idx1};
    if (item_cur->tag == BZLA_FP_TO_FP_UNSIGNED_TAG_SMT2)
    {
      ...
      exp = bitwuzla_mk_term_indexed(
          BITWUZLA_KIND_FP_TO_FP_FROM_UBV, 2, args, 2, idxs);
    }
    else
    {
      ...
      if (bitwuzla_sort_is_bv(s))
      {
        /* (_ to_fp eb sb) RoundingMode (_ BitVec m) */
        exp = bitwuzla_mk_term_indexed(
            BITWUZLA_KIND_FP_TO_FP_FROM_SBV, 2, args, 2, idxs);
      }
      else if (bitwuzla_sort_is_fp(s))
      {
        /* (_ to_fp eb sb) RoundingMode (_ FloatingPoint mb nb) */
        exp = bitwuzla_mk_term_indexed(
            BITWUZLA_KIND_FP_TO_FP_FROM_FP, 2, args, 2, idxs);
      }
      else
      {
        return !perr_smt2(
                 ...
                 BitwuzlaKind kind)
{
  ...
  std::string msg = item_cur->tag == BZLA_FORALL_TAG_SMT2 ? "forall" : "exists";
  ...
  for (i = 1; i < nargs; i++)
  {
    if (item_cur[i].tag != BZLA_SYMBOL_TAG_SMT2)
    {
      ...
      return !perr_smt2(
          parser, "expected symbol as argument %d of '%s'", i, msg.c_str());
    }
  }
      ...
      && item_cur[nargs].tag != BZLA_EXP_TAG_SMT2)
  {
    ...
    return !perr_smt2(parser,
                      "expected expression as argument %d of '%s'",
                      nargs,
                      msg.c_str());
  }
  if (!bitwuzla_term_is_bool(item_cur[nargs].exp))
  {
    parser->perrcoo = item_cur[nargs].coo;
    return !perr_smt2(
        parser, "body of '%s' is not a boolean term", msg.c_str());
  }
  BZLA_INIT_STACK(parser->mem, args);
  for (i = 1; i < nargs; i++)
  {
  ...
  item_open[0].exp = bitwuzla_mk_term(kind, BZLA_COUNT_STACK(args), args.start);
  BZLA_RELEASE_STACK(args);
  parser->work.top = item_cur;
  return 1;
}
                                   ...
                                   const char *msg)
{
  ...
  if (nargs == 1)
  {
    ...
    {
      if (!parse_uint64_smt2(parser, true, &item_open->num)) return 0;
    }
  }
  else if (tag == BZLA_FP_TO_FP_TAG_SMT2
  ...
  {
    assert(nargs == 2);
    if (!parse_uint64_smt2(parser, true, &item_open->idx0)) return 0;
    if (item_open->idx0 <= 1)
    {
      return !perr_smt2(
          parser,
    ...
    if (!parse_uint64_smt2(parser, true, &item_open->idx1)) return 0;
    if (item_open->idx1 <= 1)
    {
      return !perr_smt2(
          parser,
  ...
  item_open->tag   = static_cast<BzlaSMT2Tag>(tag);
  item_open->node  = node;
  parser->work.top = item_cur;
    ...
    BitwuzlaTerm (*fun)(BitwuzlaSort))
{
  ...
  BitwuzlaSort sort = bitwuzla_mk_fp_sort(item_open->idx0, item_open->idx1);
  BitwuzlaTerm exp  = fun(sort);
                               ...
                               BzlaSMT2Item *item_cur)
{
  ...
  {
    item_cur->tag = BZLA_EXP_TAG_SMT2;
    item_cur->exp = bitwuzla_mk_true();
  }
  else if (tag == BZLA_FALSE_TAG_SMT2)
  {
    item_cur->tag = BZLA_EXP_TAG_SMT2;
    item_cur->exp = bitwuzla_mk_false();
  }
...
parse_open_term(BzlaSMT2Parser *parser, int32_t tag)
{
  ...
  item_cur = push_item_smt2(parser, static_cast<BzlaSMT2Tag>(tag));
  ...
  if (tag == BZLA_LPAR_TAG_SMT2)
  {
    ...
    else if (parser->sorted_var)
    {
      ...
      q        = push_item_smt2(parser, BZLA_SYMBOL_TAG_SMT2);
      q->node  = sym;
      sym->exp = bitwuzla_mk_var(s, sym->name);
    }
    parser->open++;
  }
  ...
  {
    item_cur->tag = BZLA_EXP_TAG_SMT2;
    s             = bitwuzla_mk_bv_sort(strlen(parser->token.start + 2));
    item_cur->exp = bitwuzla_mk_bv_value(s, parser->token.start + 2, 2);
  }
                    ...
                    BitwuzlaSort *resptr)
{
  ...
  int32_t tag;
  uint64_t width = 0, width_eb = 0, width_sb = 0;
  std::string msg;
  ...
  {
    if (!parse_uint64_smt2(parser, true, &width_eb)) return 0;
    if (width_eb <= 1)
    {
      return !perr_smt2(
          parser,
    ...
    if (!parse_uint64_smt2(parser, true, &width_sb)) return 0;
    if (width_sb <= 1)
    {
      return !perr_smt2(
          parser,
    ...
    //          width_eb,
    //          width_sb);
    *resptr = bitwuzla_mk_fp_sort(width_eb, width_sb);
  ...
  else
  {
    ...
    //          width);
    *resptr = bitwuzla_mk_bv_sort(width);
    msg     = " to close bit-vector sort";
  }

  BZLA_PUSH_STACK(parser->sorts, *resptr);

  return read_rpar_smt2(parser, msg.c_str());
}

static int32_t
parse_array_sort(BzlaSMT2Parser *parser, int32_t tag, BitwuzlaSort *sort)
{
  BitwuzlaSort index, value;
  if (tag == BZLA_ARRAY_TAG_SMT2)
  {
    tag = read_token_smt2(parser);
...
define_fun_smt2(BzlaSMT2Parser *parser)
{
  ...
  do
  {
    ...
    if (tag != BZLA_RPAR_TAG_SMT2)
    {
      ...
      if (!parse_sort(parser, tag, false, &s)) return 0;
      nargs++;
      arg->exp   = bitwuzla_mk_var(s, arg->name);
  ...
  if (nargs)
  {
    ...
    tmp = bitwuzla_mk_term(
        BITWUZLA_KIND_LAMBDA, BZLA_COUNT_STACK(args), args.start);
    if (parser->commands.model)
    {
      ...
      eq = bitwuzla_mk_term2(BITWUZLA_KIND_EQUAL, fun->exp, tmp);
      bitwuzla_assert(get_bitwuzla(parser), eq);
    }
    else
    {
      fun->exp = tmp;
  ...
  else
  {
    if (parser->commands.model)
    {
      ...
      eq = bitwuzla_mk_term2(BITWUZLA_KIND_EQUAL, fun->exp, exp);
      bitwuzla_assert(get_bitwuzla(parser), eq);
    }
    else
    {
...
set_option_smt2(BzlaSMT2Parser *parser)
{
  ...
  else
  {
    ...
    else
    {
      ...
      o   = bitwuzla_get_option_from_string(parser->options, opt);
...
{
  assert(!parser->error);
  Bitwuzla *bitwuzla = get_bitwuzla(parser);
  BZLA_RESET_STACK(parser->sat_assuming_assumptions);
  if (parser->commands.check_sat++
      && (true
          || !bitwuzla_get_option(parser->options, BITWUZLA_OPT_INCREMENTAL)))
  {
    // BZLA_MSG(bitwuzla_get_bzla_msg(bitwuzla),
    //          1,
  ...
  {
    parser->res->result =
        bitwuzla_check_sat_assuming(bitwuzla, assc, assumptions);
  }
  else
  {
...
static int32_t
read_exp_list(BzlaSMT2Parser *parser, BitwuzlaTermStack *exps, BzlaSMT2Coo *coo)
{
  int32_t tag      = 0;
  BitwuzlaTerm exp = 0;
...
read_command_smt2(BzlaSMT2Parser *parser)
{
  ...
  switch (tag)
  {
    ...
    case BZLA_GET_UNSAT_CORE_TAG_SMT2: {
      ...
      fputc('(', parser->outfile);
      size_t size;
      unsat_core = bitwuzla_get_unsat_core(get_bitwuzla(parser), &size);
      const char *sym;
    ...
    case BZLA_MODEL_TAG_SMT2:
      ...
      while (read_command_smt2(parser)
             && !bitwuzla_terminate(get_bitwuzla(parser)))
        ;
      break;
