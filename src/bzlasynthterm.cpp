...
init_next_sort(BzlaCartProdIterator *it)
{
  ...
  for (; i < it->e0_exps->size; i++)
  {
    ...
    if (key)
    {
      ...
      it->e0_stack =
          static_cast<BzlaNodePtrStack *>(it->e0_exps->data[i].as_ptr);

      d = bzla_hashint_map_get(it->e1_exps, key);
      if (d)
        it->e1_stack = static_cast<BzlaNodePtrStack *>(d->as_ptr);
      else
        break;
      return;
    }
...
add_exp(Bzla *bzla, uint32_t exp_size, Candidates *candidates, BzlaNode *exp)
{
  ...
  if (exp_size >= BZLA_COUNT_STACK(candidates->exps))
  {
    ...
    sorted_exps = static_cast<BzlaIntHashTable *>(
        BZLA_PEEK_STACK(candidates->exps, exp_size));

  d = bzla_hashint_map_get(sorted_exps, sort);
  if (d)
    exps = static_cast<BzlaNodePtrStack *>(d->as_ptr);
  else
  {
    BZLA_CNEW(mm, exps);
  ...
  for (cur_level = 2; !max_level || cur_level < max_level; cur_level++)
  {
    ...
    for (i = 0; i < nops; i++)
    {
      if (ops[i].arity == 1)
      {
         ...
         * operators */
        e0_exps = static_cast<BzlaIntHashTable *>(
            BZLA_PEEK_STACK(candidates.exps, cur_level - 1));
        for (j = 0; j < e0_exps->size; j++)
        {
          if (!e0_exps->keys[j]) continue;
          exps = static_cast<BzlaNodePtrStack *>(e0_exps->data[j].as_ptr);
      ...
      else if (ops[i].arity == 2)
      {
        ...
        while (bzla_has_next_part_gen(&pg))
        {
          ...
          e0_exps = static_cast<BzlaIntHashTable *>(
              BZLA_PEEK_STACK(candidates.exps, tuple[0]));
          e1_exps = static_cast<BzlaIntHashTable *>(
              BZLA_PEEK_STACK(candidates.exps, tuple[1]));
      ...
      else if (cur_level > 2)
      {
        ...
        while (bzla_has_next_part_gen(&pg))
        {
          ...
          e1_exps = static_cast<BzlaIntHashTable *>(
              BZLA_PEEK_STACK(candidates.exps, tuple[1]));
          e2_exps = static_cast<BzlaIntHashTable *>(
              BZLA_PEEK_STACK(candidates.exps, tuple[2]));
...
DONE:
  ...
  {
    e0_exps =
        static_cast<BzlaIntHashTable *>(BZLA_PEEK_STACK(candidates.exps, i));
    for (j = 0; j < e0_exps->size; j++)
    {
  ...
  while (!BZLA_EMPTY_STACK(value_caches))
  {
    ...
    for (j = 0; j < value_cache->size; j++)
    {
      ...
      bzla_bv_free(mm,
                   static_cast<BzlaBitVector *>(value_cache->data[j].as_ptr));
    }
