...
{
  // `NodeManager::release` defers destruction until the last `NodeData` is
  // freed, so all hash-consed nodes are already gone by the time we run.
  assert(d_num_elements == 0);
}
