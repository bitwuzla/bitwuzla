...
#define BZLA_ABORT_OLD(cond, msg...)                                \
  do                                                                \
  {                                                                 \
...
#define BZLA_ABORT_ARG_NULL(arg)                                 \
  do                                                             \
  {                                                              \
    BZLA_ABORT_OLD((arg) == 0, "'%s' must not be NULL\n", #arg); \
  } while (0)

#define BZLA_ABORT_REFS_NOT_POS(arg)                              \
  do                                                              \
  {                                                               \
    BZLA_ABORT_OLD(bzla_node_real_addr(arg)->ext_refs < 1,        \
...
#define BZLA_ABORT_BZLA_MISMATCH(argbtor, argnode)                            \
  do                                                                          \
  {                                                                           \
    BZLA_ABORT_OLD(bzla_node_real_addr(argnode)->bzla != (argbtor),           \
...
#define BZLA_ABORT_IS_NOT_ARRAY(arg)                                           \
  do                                                                           \
  {                                                                            \
    BZLA_ABORT_OLD(!bzla_node_is_array(arg), "'%s' must be an array\n", #arg); \
  } while (0)
...
#define BZLA_ABORT_IS_NOT_RM(arg)                                      \
  do                                                                   \
  {                                                                    \
    BZLA_ABORT_OLD(!bzla_sort_is_rm(bzla, bzla_node_get_sort_id(arg)), \
...
#define BZLA_ABORT_IS_NOT_FUN(arg)                                      \
  do                                                                    \
  {                                                                     \
    BZLA_ABORT_OLD(!bzla_sort_is_fun(bzla, bzla_node_get_sort_id(arg)), \
...
#define BZLA_ABORT_IS_FUN_OR_ARRAY(arg)                              \
  ...
  {                                                                  \
    BZLA_ABORT_OLD(                                                  \
        bzla_sort_is_fun(bzla, bzla_node_get_sort_id(arg))           \
...
#define BZLA_ABORT_SORT_MISMATCH(argbw1, argbw2)                        \
  ...
  {                                                                     \
    BZLA_ABORT_OLD(                                                     \
        bzla_node_get_sort_id(argbw1) != bzla_node_get_sort_id(argbw2), \
