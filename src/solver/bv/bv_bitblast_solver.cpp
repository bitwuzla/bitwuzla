...
BvBitblastSolver::solve()
{
  ...
  // Assume last, after everything that may throw. The SAT solver buffers
  // assumptions until solve() and only clears them there, so an exception in
  // between would leak them into the next solve() call.
...
BvBitblastSolver::pop()
{
  ...
  // The manager decrements its level counter after invoking the callbacks,
  // so d_mgr->num_levels() is the level currently being popped. Levels above
  // d_sat_level were never materialized in the SAT solver (sync_sat_level()
  // is only called for levels with pending assertions), so there is nothing
  // to pop for them.
...
BvBitblastSolver::init_sat_solver()
{
  ...
  // A newly created SAT solver/CNF encoder has no activation levels.
  d_sat_level = 0;
}

void
BvBitblastSolver::sync_sat_level(size_t level)
{
