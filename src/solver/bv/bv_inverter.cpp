                       ...
                       size_t idx)
{
  ...
  switch (predicate)
  {
    ...
    case Kind::BV_UGE:
      // x >>a s >=_u t
      // IC: (and (not (and (bvuge s t) (bvslt s 0_[w]))) (not (= t 0_[w])))
      if (idx == 0)
      {
        Node zero = d_nm.mk_value(BitVector::mk_zero(bw));
        return d_nm.mk_node(
    ...
    case Kind::BV_SGE: {
      ...
      return d_nm.mk_node(
          ...
          {d_nm.mk_node(
              Kind::AND,
              {d_nm.mk_node(Kind::BV_ULT, {t, d_nm.mk_node(Kind::NOT, {t})}),
               d_nm.mk_node(Kind::BV_SLT, {s, t})})});
    }
                       ...
                       size_t idx)
{
  ...
  const Node& x = nodes[idx];
  const Node& t = nodes.back();
  uint64_t bw   = t.type().bv_size();
  uint64_t bw_x = x.type().bv_size();
  uint64_t n    = t.type().bv_size() - bw;
  switch (predicate)
  {
    case Kind::BV_ULT:
      // x<n> <_u t
