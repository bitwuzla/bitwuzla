...
SolverEngine::term_level(const Node& term)
{
  ...
  {
    const Node& cur = visit.back();
    // Level already computed (in this or a previous call).
    ...
    if (inserted)
    {
      // First visit: queue children.
      visit.insert(visit.end(), cur.begin(), cur.end());
      continue;
    }
    // Second visit: all children are computed, the level is the maximum over
    // the children's levels. Leaf terms that are not values take the assertion
    // level at which they were first registered (top level if not registered).
    uint32_t level = 0;
