  ...
  if (d_inverse == nullptr)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          uint64_t clo_s = s.count_leading_ones();
          uint64_t clo_t = t.count_leading_ones();
          assert(clo_t >= clo_s);
          uint64_t shift = clo_t - clo_s;
          if (t.is_ones())
          {
            assert(!x.has_fixed_bits());
            d_inverse.reset(new BitVector(size,
                                     ...
                                     BitVector& res_max_hi)
{
  ...
  BitVector min, max;        // the unsigned bounds wrt. s and t
...
{
  BitVector min_lo, max_lo, min_hi, max_hi;
  compute_min_max_bounds(s, t, pos_x, min_lo, max_lo, min_hi, max_hi);
...
BitVectorUlt::inverse_value_concat(bool t, uint32_t pos_x, uint32_t pos_s)
{
  ...
  uint64_t bw_x0 = op_x[0]->size();
  uint64_t bw_x1 = op_x[1]->size();
                                     ...
                                     BitVector& res_max_hi)
{
  ...
  BitVector min, max;        // the signed bounds wrt. s and t
