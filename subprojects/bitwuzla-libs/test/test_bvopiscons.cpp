...
class TestBvOpIsCons : public TestBvDomainCommon
{
 protected:
  void SetUp() override
  {
...
bool
TestBvOpIsCons::check_sat_binary(Kind kind,
                                 const BitVectorDomain& x,
                                 const BitVector& t,
                                 uint32_t pos_x)
{
  BitVectorDomainGenerator genx(x), gens(BitVectorDomain(x.size()));
  do
  {
    BitVector val = genx.has_next() ? genx.next() : x.lo();
    while (gens.has_next())
    {
      BitVector s = gens.next();
      BitVector res;
      switch (kind)
      {
...
template <class T>
void
TestBvOpIsCons::test_binary(Kind kind, uint32_t pos_x, bool const_bits)
{
  ...
  for (const std::string& x_value : x_values)
  {
    ...
    for (uint32_t i = 0; i < nval_s; i++)
    {
      ...
      for (uint32_t j = 0; j < nval_t; j++)
      {
        ...
        bool res    = op.is_consistent(t, pos_x);
        bool status = check_sat_binary(kind, x, t, pos_x);
        if (res != status)
        {
