...
main()
{
  ...
  BitwuzlaTerm *one = bitwuzla_mk_bv_one(bzla, bv8);
  // alternatively, you can create bit-vector value one with:
  // BitwuzlaTerm *one =
  //     bitwuzla_mk_bv_value(bzla, bv8, "1", BITWUZLA_BV_BASE_BIN);
  // BitwuzlaTerm *one = bitwuzla_mk_bv_value_uint64(bzla, bv8, 1);
  BitwuzlaTerm *two  = bitwuzla_mk_bv_value_uint64(bzla, bv8, 2);
