...
if __name__ == '__main__':
    ...
    a0 = mk_const(sortbool, 'a0')
    assumption0 = mk_term(Kind.BV_ULT, [x2, mk_term(Kind.APPLY, [f2, fpzero])])
    bitwuzla.assert_formula(mk_term(Kind.EQUAL, [a0, assumption0]))
    # (assert (! (= x1 x2 x3) :named a1))
    a1          = mk_const(sortbool, 'a1')
    assumption1 = mk_term(Kind.EQUAL, [x1, x2, x3])
    bitwuzla.assert_formula(mk_term(Kind.EQUAL, [a1, assumption1]))
    # (assert (!(= x4 ((_ to_fp_unsigned 5 11) RNE x3)) :named a2))
    a2          = mk_const(sortbool, 'a2')
    assumption2 = mk_term(Kind.EQUAL,
                               [x4,
